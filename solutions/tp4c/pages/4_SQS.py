"""Page SQS : valide la file creee par le module modules/sqs."""
...
st.set_page_config(page_title="SQS - Module Terraform", page_icon=":envelope:")
st.title("SQS - Ressource du module modules/sqs")

url = queue_url()
client = sqs_client()

if not url:
    st.error("`SQS_QUEUE_URL` est vide dans `.env`.")
    st.stop()
