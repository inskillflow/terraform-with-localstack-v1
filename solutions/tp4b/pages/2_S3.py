"""Page S3 : valide le bucket cree par le module modules/s3."""
...
st.set_page_config(page_title="S3 - Module Terraform", page_icon=":package:")
st.title("S3 - Ressource du module modules/s3")

bucket = bucket_name()
client = s3_client()

if not bucket:
    st.error("`S3_BUCKET_NAME` est vide dans `.env`.")
    st.stop()
