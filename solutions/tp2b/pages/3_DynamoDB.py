"""Page DynamoDB : valide la table creee par Terraform et permet CRUD basique."""
...
st.set_page_config(page_title="DynamoDB - Ressource Terraform", page_icon=":card_index_dividers:")
...
if not table:
    st.error("`DYNAMODB_TABLE_NAME` est vide dans `.env`. Completez `.env` d'abord.")
    st.stop()
...
with col1:
    if _table_exists(table):
        ...
        st.error(
            ...
            "Avez-vous execute `terraform apply` dans `terraform/` ?"
        )
        st.stop()
...
with st.form("add_student", clear_on_submit=True):
    student_id = st.text_input("student_id (cle primaire)", value="S001")
    first_name = st.text_input("first_name", value="")
    last_name = st.text_input("last_name", value="")
    submitted = st.form_submit_button("Ajouter")
    if submitted:
        if not student_id:
            st.error("`student_id` est obligatoire.")
        else:
            ...
            try:
                ...
                st.error(f"Erreur a l'insertion : {exc}")
...
if not items:
    ...
    rows = []
    for item in items:
        rows.append({k: list(v.values())[0] for k, v in item.items()})
