...
# TP 5 (version `c`) — Gérer plusieurs environnements Terraform et les valider avec Streamlit (sans token)

> **Version `c` — sans compte ni token.** Cette version suppose que vous avez fait les TPs 1c à 4c (bypass `LOCALSTACK_ACKNOWLEDGE_ACCOUNT_REQUIREMENT=1`). Sinon, basculez sur [`05b-...md`](05b-Chapitre5-Pratique-05-environnements-dev-test-terraform-validation-streamlit.md).
>
...
>
> **Prérequis :** les TPs 1 à 4 (version `c`) doivent être réalisés. Le projet utilise déjà des modules Terraform pour S3, DynamoDB et SQS.  
>
